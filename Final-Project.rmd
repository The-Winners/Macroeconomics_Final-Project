...
i) A strict focus on inflation targeting may increase volatility elsewhere in the economy.
ii) On the other hand an approach to inflation targeting which is too flexible may undermine credibility in the target. 
...
$$(1) \space\alpha _{t+1} =d_t +T_t ·\alpha _t + H_t \cdot \eta _t$$ 

$$ (2) \space y_t=c_t+Z_t\cdot\alpha _t + G_t\cdot \epsilon _t$$
where $\eta _t \sim N(0,Qt)$ and $\epsilon _t \sim N(0,Ht)$.
$a_0$ A vector giving the initial value/estimation of the state variable.
$P_0$ A matrix giving the variance of a0.
$d_t$ A matrix giving the intercept of the transition equation.
$c_t$ A matrix giving the intercept of the measurement equation.
$T_t$ An array giving the factor of the transition equation.
$Z_t$ An array giving the factor of the measurement equation.
$HH_t$ An array giving the variance of the innovations of the transition equation.
$GG_t$ An array giving the variance of the disturbances of the measurement equation.
$y_t$ A matrix containing the observations. “NA”-values are allowed.
